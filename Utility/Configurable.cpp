...
void Configurable::ReadManager(const string& TextPath) {
    ...
    if(file.is_open()){
        ...
        file >> Trash;
        file >> score;
        pManager=new GameManager(score,lives);
        file.close();
    }else{
...
//Collision
Vector2D Configurable::FlippersCollision(Ball &ball, float collision_time, GameManager &manager) {
    Vector2D Acceleration={0,0};
    for (int i = 0; i <FlippersNo; i++) {
        Acceleration+=pFlippers[i]->collideWith(ball,collision_time,manager).ballSpeed;
    }
    return Acceleration;
}

Vector2D Configurable::PopBumpersCollision(Ball &ball, float collision_time, GameManager &manager) {
    Vector2D Acceleration={0,0};
    for (int i = 0; i <PopBumpersNo; i++) {
        Acceleration+= pPopBumpers[i]->collideWith(ball,collision_time,manager).ballSpeed;
    }
    return Acceleration;
}

Vector2D Configurable::BallsCollision(Ball &ball, float collision_time, GameManager &manager) {
    Vector2D Acceleration={0,0};
    for (int i = 0; i <BallsNo; i++) {
...
Vector2D Configurable::WallsCollision(Ball &ball, float collision_time, GameManager &manager) {
    Vector2D Acceleration={0,0};
    for (int i = 0; i <WallsNo; i++) {
        Acceleration+=pWalls[i]->collideWith(ball,collision_time,manager).ballSpeed;
    }
    return Acceleration;
}
